...
from lex import tokens, print_tokens, lexer,get_tokens
...
def p_expresion_concatenacion(p):
    '''expresion_concatenacion : CADENA_TEXTO
...
def p_error(p):
    if p:
        print(f"\nError de sintaxis en línea {p.lineno}, posición {p.lexpos}: token inesperado '{p.value}'")
    else:
